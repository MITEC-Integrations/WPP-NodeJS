...
export class ShapeValidator<T extends object> implements IValidator<T> {
  ...
  go: (value: unknown) => Result<T> = (value) => {
    ...
    if (value === null || value === undefined) {
      ...
      return typeValidResult('Value must be an object but was an array.', this.optional);
    } else if (typeof value !== 'object') {
      return typeValidResult(`Value must be an object but was ${typeof value}.`, this.optional);
    }
